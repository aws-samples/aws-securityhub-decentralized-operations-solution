...
def lambda_handler(event, context):
    ...
    enrichment_finding_product_arn = ""
    #log the event
    ...
    enrichment_finding_product_arn = finding["ProductArn"]
    account_id = str(finding['AwsAccountId'])
    logger.debug("Finding ID: %s " , enrichment_finding_id + " and product arn " + enrichment_finding_product_arn)
    try:
            ...
            FindingIdentifiers=[
                {
                    ...
                    'ProductArn': enrichment_finding_product_arn
                }
            ],
            Note={
        ...
        if response["UnprocessedFindings"]:
            ...
            logger.info("successfully posted note to finding: %s" , enrichment_finding_product_arn + "API response: " + str(response))
    return {
